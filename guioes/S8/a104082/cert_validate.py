...
def validate_cert(cert_filepath: str, cert_common_name: str, ca_filepath: str) -> bool:
    try:
        ca_cert = cert_load(ca_filepath)

        cert = cert_load(cert_filepath)
        cert.verify_directly_issued_by(ca_cert)
        cert_validtime(cert)
        cert_validsubject(cert, [(x509.NameOID.COMMON_NAME, cert_common_name)])
...
#     try:
#         cert = cert_load("test.crt")
#         # obs: pressupõe que a cadeia de certifica só contém 2 níveis
#         cert.verify_directly_issued_by(ca_cert)
#         # verificar período de validade...
#         cert_validtime(cert)
#         # verificar identidade... (e.g.)
#         cert_validsubject(cert, [(x509.NameOID.COMMON_NAME, "User 1 (SSI Vault Client 1)")])
...
#         return False
#
# valida_certALICE(cert_load("VAULT_CA.crt"))