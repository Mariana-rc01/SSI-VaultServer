...
def cbcmac_lengthextension_example(m1, m2):
    ...
    #print("Isto é o M1: ", m1)
    #print("Isto é o M2: ", m2)
    #print("Isto é TAG1: ", tag1.hex())
    #print("Isto é TAG2: ", tag2.hex())
    
    # create a m3 (based on m1 and m2 that verifies with tag2)
    padder = padding.PKCS7(16 * 8).padder()
    m1 = padder.update(m1) + padder.finalize()

    # m2_first_bytes is the XOR of the first block of m2 and tag1
    m2_first_bytes = bytes(a ^ b for a,b in zip(m2[:16], tag1))
    # m3 is the concatenation of padded m1, m2_first_bytes, and the rest of m2
    m3 = m1 + m2_first_bytes + m2[16:]
